...
1. **Clone and install**
   ...
   Node 22.12 or newer, the floor Astro sets. `.nvmrc` pins the major, so
   `nvm use` gets you the version CI runs. Dependency versions are pinned
   exactly, so a clone builds the same way tomorrow as it does today.
