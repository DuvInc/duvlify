 ...
 *
 * The outline reads raw Markdown, so `### **2. Update the DNS**` yielded a path
 * of `****2. Update the DNS****` — syntax leaking into a JSON string field that
 * nothing renders, straight into what an agent reads and may quote.
 *
 * It also broke the anchor. `slugs` is keyed by Astro's *rendered* heading text,
 * which has no asterisks, so every decorated heading missed the lookup and fell
 * back to a computed slug. That mostly agreed by luck — `slugify` drops the
 * punctuation anyway — but not where a page repeats a heading: the renderer
 * disambiguates the second one as `…-1`, and the fallback always produced the
 * first. A deep link to the second occurrence silently pointed at the first.
 *
 ...
 */
const plainHeading = (markdown: string) =>
  markdown
    .replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
    .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1')
    .replace(/`([^`]+)`/g, '$1')
